...
function parseSprites(sprites) {
    const data = sprites.reduce((result, sprite) => {
        ...
        const _dir = path.dirname(escaped_image).replace(/^\.retina_tmp\//, "");
...
module.exports = function (options) {
    ...
    function handler(data) {
        ...
        scss.push(`$__sprite-local-group__:  ${parseSprites(data.sprites)};`);
        if (data.retina_sprites) {
            scss.push(`$__sprite-local-group-2x__: ${parseSprites(data.retina_sprites)};`);
        }
